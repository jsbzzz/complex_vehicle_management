...
class _VehicleInsertScreenState extends State<VehicleInsertScreen> {
      ...
      body: Padding(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              TextField(controller: _vNumController,
                  decoration: const InputDecoration(labelText: '차량 번호 (7~8자)')),
              TextField(controller: _vModelController,
                  decoration: const InputDecoration(labelText: '차량 모델')),

              const SizedBox(height: 20),
              const Text(
                  '소유주 정보', style: TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 10),

              // 💡 입주민 정보가 연동되지 않은 경우에만 이름/번호 입력 필드 표시
              if (_selectedVillager == null) ...[
                TextField(controller: _ownerNameController,
                    decoration: const InputDecoration(
                        labelText: '소유주 이름 (모를 경우 비움)')),
                TextField(controller: _ownerPhoneController,
                    decoration: const InputDecoration(
                        labelText: '소유주 전화번호 (모를 경우 비움)')),
              ] else
                ...[
                  ...
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('연동된 입주민: ${_selectedVillager!.name}',
              ...
              OutlinedButton.icon(
                ...
                label: Text(
                    _selectedVillager == null ? '기존 입주민 연동하기' : '다른 입주민으로 변경'),
                onPressed: () async {
                  final result = await Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => const VillagerSearchDelegate())
                  );
                  if (result != null) setState(() =>
                  _selectedVillager = result);
                },
              ),
              if (_selectedVillager != null)
                TextButton(
  ...
  void _saveVehicle() async {
    // 💡 데이터베이스 대신 레포지토리를 가져옵니다.
    final repository = Provider.of<VehicleRepository>(context, listen: false);

    if (_vNumController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('차량 번호를 입력해주세요.')));
      return;
    }

    // 💡 레포지토리의 등록 메서드 호출 (로컬 저장 + 원격 업로드 시도)
    await repository.registerVehicle(
      VehicleTableCompanion.insert(
        ...
        ownerPhone: Value(
            _selectedVillager?.phoneNumber ?? _ownerPhoneController.text),
    ...
    if (mounted) {
      ...
      mainNavKey.currentState?.changeTab(0); // '조회' 탭으로 이동
    }
  }
}